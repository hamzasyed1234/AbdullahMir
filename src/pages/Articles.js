  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-10">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto z-10">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 bg-gray-100 hover:bg-gray-200 rounded-full p-2 transition z-20"
        >
          <X size={18} />
        </button>
        {article.image_url && (
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full font-sans text-sm transition ${
                liked
                  ...
                  : 'bg-gray-50 text-gray-500 border border-gray-200 hover:bg-red-50 hover:text-red-400'
              }`}
            >
...
export default function Articles() {
  ...
  const fetchLikes = async () => {
    ...
    if (data) setLikedArticles(new Set(data.map(l => l.article_id)))
  }
  ...
  const handleLike = async (articleId) => {
    ...
    if (already) {
      await supabase.from('article_likes').delete()
        .eq('article_id', articleId).eq('session_id', sessionId)
          ...
          <div>
            {articles.map((article, i) => {
              ...
              return (
                <div
                  key={article.id}
                  className="rounded-2xl overflow-hidden mb-6 shadow-sm cursor-pointer hover:shadow-md transition-shadow"
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full transition font-sans ${
                            liked
                              ...
                              : isTeal
                                ? 'bg-white/10 text-white/60 border border-white/20 hover:bg-red-500/20 hover:text-red-300'
